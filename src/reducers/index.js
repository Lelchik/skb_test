...
export default function reducer(state = emptyState, { type, ...data }) {
    ...
    switch (type) {
        ...
        case DELETE_ROW_OR_COL:
            ...
            if (data.data) { //удалить столбец
                if (newState[key].col > 1) {
                    ...
                    error = 'невозможно удалить столбец! Количество столбцов у матрицы больше 1'
                    color = '#f6c1c0'
                }
            } else { //удалить строку
                if (newState[key].row > 1) {
                    ...
                    error = 'невозможно удалить строку! Количество строк у матрицы больше 1'
                    color = '#f6c1c0'
                }
            }
            console.log(newState);
        ...
        case ADD_ROW_OR_COL:
            ...
            } else { //добавить строку
                newState[key].row++
                    newState[key].array.push(Array.from({ length: newState[key].col }, () => ''))
                if (key === 'matrixA') {
                    newState.matrixC.row++;
                    newState.matrixC.array.push(Array.from({ length: newState.matrixC.col }, () => ''))
                }
            }
            //console.log(newState);
            return {
        ...
        case MULTIPLY_MATRIX:
            ...
            if (state.matrixA.col !== state.matrixB.row) return {
                ...state,
                error: 'количество столбцов матрицы А не равно количеству строк матрицы В',
                color: '#f6c1c0'
            }
            ...
            for (var k = 0; k < state.matrixB.col; k++) {
                for (var i = 0; i < state.matrixA.row; i++) {
                    var t = 0;
                    for (var j = 0; j < state.matrixB.row; j++) t += state.matrixA.array[i][j] * state.matrixB.array[j][k]
